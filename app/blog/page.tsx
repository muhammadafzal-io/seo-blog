...
async function getArticles() {
  const { data, error } = await supabase
    .from('articles')
    .select(`id, keyword, meta_title, meta_description, query_score, status, updated_at`)
    .eq('status', 'published')
  ...
  return (
    <div style={s.page}>
            ...
            {articles.map((article: any, i: number) => (
              ...
              >
                <div style={s.cardTop}>
                  ...
                  {article.query_score >= 80 && (
                    <span style={s.scoreBadge}>★ {article.query_score}</span>
                  )}
                </div>
                <h2 style={s.cardTitle}>{article.meta_title || article.keyword}</h2>
      ...
      <footer style={s.footer}>
        <span style={s.footerText}>Auto-published</span>
      </footer>
